      ...
      <main className="max-w-7xl mx-auto w-full px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start flex-grow">
        ...
        <section id="milestone-content" className="lg:col-span-8 flex flex-col bg-white rounded-3xl shadow-sm border border-slate-200 p-2 scroll-mt-24 relative z-0">
          ...
          <div className="p-4 border-t border-slate-100 flex justify-center mt-auto">
            <button 
              onClick={() => {
                document.getElementById('roadmap-aside')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
              }}
              className="px-6 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-900 text-sm font-bold rounded-xl border border-slate-200 transition-all flex items-center gap-2"
            >
              ↑ 返回学习地图
            </button>
          </div>
